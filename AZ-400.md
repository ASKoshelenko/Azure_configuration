...
**Пояснение:**  
Раздел Users показывает наиболее часто посещаемые страницы и действия пользователей.
...
**Correct roles:**  
- **User1 (view log data):** ✅ `Log Analytics Reader`  
- **User2 (edit log alerts):** ✅ `Monitoring Contributor`

**Explanation:**  
- `Log Analytics Reader` allows querying and viewing logs without permission to change anything.  
- `Monitoring Contributor` can manage alerts and monitoring settings but not broader resources.

---

**Русский (перевод):**  
✅ Правильные роли:  
- User1 (просмотр логов): ✅ Log Analytics Reader  
- User2 (управление алертами): ✅ Monitoring Contributor

**Вопрос:**  
Какие роли минимально необходимы User1 и User2, чтобы:
- User1 — только просматривал логи  
- User2 — редактировал оповещения (alerts)

**Пояснение:**  
Reader — только чтение, Contributor — управление, но без администрирования ресурсов.
...
**Correct widget mapping:**  
- **Time to close a work item** → ✅ `Lead Time`  
- **Completed backlog items** → ✅ `Sprint Burndown`  
- **Restore failed services time** → ✅ `Mean Time to Restore (MTTR)`

**Explanation:**  
- `Lead Time` — от открытия до закрытия задачи  
- `Sprint Burndown` — количество выполненных задач  
...
**Русский (перевод):**  
✅ Правильные соответствия:
- Время до закрытия задачи → ✅ Lead Time  
- Кол-во завершённых задач → ✅ Sprint Burndown  
- Время восстановления → ✅ MTTR

**Вопрос:**  
Какой тип виджета выбрать на дашборде для:
- Закрытия задач  
- Выполненных backlog-элементов  
- Восстановления после сбоев

**Пояснение:**  
Каждый виджет отображает ключевой аспект DevOps-показателей.
...
**Explanation:**  
The **Availability and Performance** section in App Service diagnostics shows downtime history and availability trends, making it the fastest way to check offline status.

---

**Русский (перевод):**  
✅ Правильный ответ: D — Availability and Performance

**Вопрос:**  
Как быстро определить, когда приложение App1 было недоступно, с минимальными трудозатратами?

**Варианты:**  
A. Navigator  
B. Конфигурация и управление  
C. Диагностические инструменты  
D. ✅ Доступность и производительность

**Пояснение:**  
Раздел Availability покажет все инциденты недоступности за заданный период.

---

✅ Q99  
**English (Original):**  
You have an Azure Kubernetes Service (AKS) cluster AKS1.  
You collect and analyze metrics using the Azure Monitor managed service for Prometheus.  
You need to analyze AKS1 performance.
...
**Explanation:**  
Azure Monitor for Prometheus supports **PromQL**, the Prometheus Query Language. It is specifically designed for metric queries.

---

**Русский (перевод):**  
✅ Правильный ответ: B — PromQL

**Вопрос:**  
Вы используете Azure Monitor с поддержкой Prometheus. Какой язык запросов использовать для анализа метрик?
...
**Пояснение:**  
PromQL — родной язык для Prometheus-метрик и используется в Azure Monitor Managed Prometheus.
...
You execute the query shown in the exhibit.
...
**Correct answers:**  
- This query returns: ✅ `number of 500 errors per hour`  
- The aggregation function used is: ✅ `count()`

**Explanation:**  
The `summarize count()` groups results by 1-hour bins and counts the number of HTTP 500 errors.

---

**Русский (перевод):**  
✅ Правильные утверждения:  
- Запрос возвращает: ✅ количество ошибок 500 по часам  
- Используемая функция агрегации: ✅ count()

**Вопрос:**  
Что делает данный KQL-запрос?

**Пояснение:**  
Фильтрует по коду ошибки `500`, агрегирует по часам, считает количество таких ошибок в каждом временном интервале.
