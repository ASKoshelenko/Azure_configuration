...
3. Select **Developer Tools > Application Insights**.  
4. In the **Create Application Insights** page:
   - **Name**: `az400-9940427-main`  
   - **Application Type**: Choose **ASP.NET**, or select what matches your app.  
   - **Resource Group**: Choose the group where the app resides.  
   - **Region**: Match the region of your web app.  
5. Click **Review + Create**, then **Create**.

---

#### 🟢 Step 2: Link Web App with Application Insights
1. In Azure Portal, go to **App Services** > open your app `az400-9940427-main`.  
2. In the **Monitoring** section, select **Application Insights**.  
3. Click **Turn on Application Insights**.  
4. Choose **"Use existing"**, then select `az400-9940427-main`.  
5. Click **Apply** or **OK**.

This connects your web app to the created Application Insights instance for telemetry collection.

---

**Русский (перевод):**

✅ Создание Application Insights и подключение веб-приложения:

1. В Azure Portal создайте **Application Insights** с именем `az400-9940427-main`.  
2. Откройте веб-приложение `az400-9940427-main` в разделе **App Services**.  
3. В разделе **Monitoring → Application Insights** выберите "Включить".  
4. Выберите созданный экземпляр и нажмите **ОК**.

Это позволит веб-приложению отправлять телеметрию (запросы, ошибки, задержки) в Application Insights.

---

📌 После подключения вы сможете настраивать алерты и выполнять аналитику на основе журналов в App Insights.
...
**Explanation:**  
Box 1: **livenessProbe**  
Liveness probes determine whether the application inside a container is still running. If the liveness probe fails, the kubelet kills the container, and the container is subjected to its restart policy.

Box 2: **periodSeconds: 15**  
The `periodSeconds` property defines how often (in seconds) to perform the probe. In this case, every 15 seconds means four checks per minute, as required.

---

**Русский (перевод):**  
✅ Правильный ответ: **Box 1: livenessProbe**, **Box 2: periodSeconds: 15**

**Вопрос:**  
У вас есть под в службе Azure Kubernetes Service (AKS).  
Вам нужно настроить пробу, чтобы выполнить следующие действия:

- Убедиться, что под отвечает на сервисные запросы. Проверять статус пода **четыре раза в минуту**.  
- Инициировать завершение работы, если под не отвечает.

**Как следует завершить YAML-файл конфигурации?**
...
Box 2:  
A. initialDelaySeconds: 15  
B. periodSeconds: 15 — ✅  
C. timeoutSeconds: 15  

**Пояснение:**  
Box 1: `livenessProbe` используется для определения, жив ли контейнер. Если проверка не проходит, контейнер будет перезапущен. Это как раз то поведение, которое нужно по условию.  
Box 2: `periodSeconds: 15` означает, что проверка будет выполняться каждые 15 секунд, то есть 4 раза в минуту — как указано в задании.
...
**Вопрос:**  
В вашей компании используются Azure SQL Database Intelligent Insights и Azure Application Insights для мониторинга.  
Вам поручено выполнять анализ мониторинга с использованием произвольных (ad-hoc) запросов. Необходимо использовать правильный язык запросов.  
**Решение:** Вы используете Azure Log Analytics.  
**Удовлетворяет ли решение цели?**
...
**Пояснение:**  
Хотя Azure Log Analytics позволяет выполнять пользовательские запросы, для анализа данных в **Azure SQL Analytics** необходим **специализированный язык и подход**, отличающийся от стандартного Log Analytics. Следовательно, использование только Azure Log Analytics **не является достаточным решением** в этом контексте.
...
**Вопрос:**  
В вашей компании используются Azure SQL Database Intelligent Insights и Azure Application Insights для мониторинга.  
Вам поручено выполнять анализ мониторинга с помощью ad-hoc запросов. Нужно использовать правильный язык запросов.  
**Решение:** Вы используете Contextual Query Language (CQL).  
**Удовлетворяет ли это решение поставленной цели?**
...
**Пояснение:**  
Для анализа данных мониторинга в Azure SQL и Application Insights следует использовать **Kusto Query Language (KQL)**, а не CQL. Поэтому предложенное решение **не соответствует цели**.
...
**Вопрос:**  
В вашей компании используются Azure SQL Database Intelligent Insights и Azure Application Insights для целей мониторинга.  
Вам поручено анализировать мониторинг с помощью ad-hoc запросов. Необходимо использовать правильный язык запросов.  
**Решение:** Вы используете Transact-SQL (T-SQL).  
...
**Пояснение:**  
Хотя Transact-SQL подходит для работы с данными в Azure SQL Database, **для анализа телеметрии в Azure Monitor и Application Insights необходимо использовать Kusto Query Language (KQL)**. Поэтому T-SQL — **неподходящее решение** для этой задачи.
...
**Русский (перевод):**  
✅ Правильный ответ: **A. Создать задачу сборки и использовать сервис WhiteSource Bolt**

**Вопрос:**  
У вашей компании есть проект в Azure DevOps, в котором используется pipeline сборки, включающий около 50 open source библиотек.  
Вам поручено убедиться, что вы можете сканировать проект на наличие типичных уязвимостей безопасности в этих open source библиотеках.  
**Какие действия следует предпринять?**

**Варианты ответа:**  
A. Создать задачу сборки и использовать сервис WhiteSource Bolt ✅  
B. Создать задачу развёртывания и использовать сервис WhiteSource Bolt  
C. Создать задачу сборки и использовать сервис Chef  
D. Создать задачу развёртывания и использовать сервис Chef

**Пояснение:**  
WhiteSource Bolt (ныне Mend Bolt) — это инструмент для сканирования open source компонентов на наличие уязвимостей. Он интегрируется в процесс сборки, а не развёртывания, что делает **build task** правильным выбором. Chef — это инструмент управления конфигурациями и не подходит для сканирования зависимостей.
...
**Русский (перевод):**  
✅ Правильный ответ: **A. ARM-шаблоны**, **B. DSC-расширение для Windows**

**Вопрос:**  
Вы создали проект Azure DevOps для нового приложения, которое будет развёрнуто на нескольких виртуальных машинах Windows Server 2016 в Azure.  
Вы готовите решение для развёртывания, которое позволит виртуальным машинам поддерживать единообразную конфигурацию и минимизировать административные затраты на настройку.  
**Что из перечисленного должно быть частью вашего решения? (Выберите два варианта)**

**Варианты ответа:**  
A. Шаблоны Azure Resource Manager ✅  
B. Расширение PowerShell Desired State Configuration (DSC) для Windows ✅  
C. Группы развёртывания в Azure pipeline  
D. Расширение Custom Script для Windows  
E. Шаблоны этапов Azure pipeline

**Пояснение:**  
ARM-шаблоны позволяют описывать инфраструктуру как код и обеспечивать единообразие конфигурации.  
DSC-расширение PowerShell применяется для автоматизации поддержания заданного состояния конфигурации виртуальных машин, что помогает минимизировать ручные настройки.
...
**Вопрос:**  
Вы планируете использовать Azure Artifacts для совместного использования пакетов, которые вы написали, протестировали, проверили и задеплоили.  
Вы хотите использовать одну общую ленту (feed) для публикации нескольких сборок каждого пакета. Также необходимо убедиться, что выпуск пакетов, находящихся в разработке, ограничен.  
**Какое из следующих действий следует предпринять?**

**Варианты ответа:**  
A. Использовать статический анализ кода  
B. Использовать представления (views) ✅  
C. Использовать динамический анализ кода  
D. Использовать внешние (upstream) источники

**Пояснение:**  
Представления (views) в Azure Artifacts позволяют управлять видимостью пакетов. Вы можете, например, настроить представление для стабильных релизов, исключая нестабильные или разрабатываемые версии, и тем самым ограничить их использование.
...
**Вопрос:**  
Вам нужно проверить подчеркнутый фрагмент, чтобы определить, является ли он корректным.  
При переходе на Azure DevOps, JIRA якобы должна быть заменена на сервис **build pipelines** в Azure DevOps.  
Если утверждение корректно, выберите **"No adjustment required"**.  
Если некорректно — выберите правильный вариант замены.

**Варианты ответа:**  
A. Никаких изменений не требуется  
B. Repos (репозитории)  
C. Release Pipelines (релизные пайплайны)  
D. Boards (доски для управления задачами) ✅

**Пояснение:**  
JIRA — это система управления задачами и проектами. В Azure DevOps эквивалентом JIRA является **Azure Boards**, а не **build pipelines**, которые относятся к CI/CD. Поэтому правильный ответ — Boards.
...
**Русский (перевод):**  
✅ Правильный ответ: **A. Red/Black deployment (развёртывание красный/чёрный)**

**Вопрос:**  
Вы разрабатываете стратегию релизов для приложения APP-01.  
Стратегия должна минимизировать время, необходимое для развёртывания новых версий приложения.  
Также она должна обеспечивать возможность быстрого отката в случае проблем.  
**Какую стратегию релизов следует использовать?**

**Варианты ответа:**  
A. Развёртывание Red/Black (два параллельных окружения с переключением трафика) ✅  
B. Пошаговое (rolling) развёртывание  
C. Развёртывание по принципу "Big Bang" (одновременно для всех)  
D. Canary (канареечное) развёртывание

**Пояснение:**  
Стратегия Red/Black (или Blue/Green) позволяет иметь два идентичных окружения.  
Вы разворачиваете новую версию в одном из них, проверяете её, и затем переключаете трафик.  
Это обеспечивает минимальное время отката — просто вернуться к предыдущему окружению.
...
**Русский (перевод):**  
✅ Правильный ответ: **A. Yes (Да)**

**Вопрос:**  
Вы выполняете команду `Register-AzureRmAutomationDscNode` в среде вашей компании.  
Вам нужно убедиться, что тестовые серверы компании остаются корректно сконфигурированными, даже при отклонении конфигурации (configuration drift).

**Решение:**  
Вы устанавливаете параметр `-ConfigurationMode` в значение `ApplyAndAutocorrect`.  
**Соответствует ли решение поставленной цели?**
...
**Пояснение:**  
Параметр `ApplyAndAutocorrect` в PowerShell DSC (Desired State Configuration) обеспечивает автоматическое применение конфигурации и исправление любых отклонений от желаемого состояния.  
Это позволяет гарантировать соответствие системы нужной конфигурации, устраняя дрейф настроек.
